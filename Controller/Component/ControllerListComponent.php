...
class ControllerListComponent extends Component
{
	/*
		Function-get()
		Arguments-
		Description- Used to get all controllers with all methods for permissions
	*/
	public function get()
	{
		...
		$parentActionsDefined=$this->removePrivateActions($parentActions);
		$parentActionsDefined = array_diff($parentActionsDefined, $superParentActions);
		$controllers= array();
		foreach($controllerClasses as $controller)
		{
			$controllername=str_replace('Controller', '',$controller);
			$actions= $this->getControllerMethods($controllername, $superParentActions, $parentActions);
			if(!empty($actions))
			{
		...
		foreach ($plugins as $p)
		{
			...
			{
				$controllername=str_replace('Controller', '',$controller);
				$actions= $this->getControllerMethods($controllername, $superParentActions, $parentActions, $p);
				if(!empty($actions))
				{
					$controllers[$controllername] = $actions;
				}
	...
	/*
		...
		Arguments-
		Description- Used to delete private actions from list of controller's methods
	*/
	function removePrivateActions($actions)
	{
	...
	/*
		...
		Arguments-
		Description- Used to get methods of controller 
	*/
	function getControllerMethods($controllername, $superParentActions, $parentActions, $p=null)
	{
		if(empty($p))
		...
		if(!empty($actions))
		{
			$actions=$this->removePrivateActions($actions);
			if($controllername=='App')
				$actions= array_diff($actions, $superParentActions);
			else
	...
	/*
		...
		Arguments-
		Description- Used to get controller's list 
	*/
	function getControllers()
	{
		$controllerClasses = App::objects('Controller');
